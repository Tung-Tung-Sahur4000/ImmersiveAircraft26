...
public class TinyTNTRenderer extends EntityRenderer<TinyTNT, TinyTNTRenderState> {
    ...
    public void extractRenderState(TinyTNT entity, TinyTNTRenderState state, float tickDelta) {
        ...
        // 26.1 submits a resolved block model rather than a raw BlockState.
        blockModelResolver.update(state.blockRenderState, Blocks.TNT.defaultBlockState(), blockDisplayContext);
    }
    ...
    public void submit(TinyTNTRenderState state, PoseStack matrixStack, SubmitNodeCollector collector, CameraRenderState cameraState) {
        ...
        matrixStack.mulPose(Axis.YP.rotationDegrees(90.0f));
        TntMinecartRenderer.submitWhiteSolidBlock(state.blockRenderState, matrixStack, collector, state.lightCoords, i / 5 % 2 == 0, state.outlineColor);
        matrixStack.popPose();
